...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/api/outline` | 入参 `{topic, level?, adjust?}`，出参章节大纲（供"大纲确认页"，用快速档模型秒出） |
| `POST` | `/api/lesson` | 入参 `{topic, level?, outline?}`，出参一份 Lesson Script JSON（给了 outline 就按它生成） |
...
体验流程：**点题 → 大纲确认页（可一句话调深浅/换角度）→ 确认 → 生成正文 → 播放**。播放支持 `🔊` 语音讲解（讲完一句才翻屏），章节切换有过渡动画。想一次看全部板书类型，点首页「🎨 板书类型画廊」。
...
**已落地**：大纲确认页（先出大纲再生成正文）· 9 类板书（含图表、数学动画）· TTS 语音讲解与板书时序对齐 · 章节切换过渡 · 课堂追问。

**下一步（从"能用"到"好用"）**：

1. **正文流式生成**（PRD 6.2）：第 1 章生成完即开播、后台续生成后续章节——把"备课中"等待也变无感（大纲已先行）。
2. **配图真生成**：把 `image` 接上图像模型（当前为画框占位）；继续扩充数学动画模板。
3. **AI 前沿主题联网检索**（PRD 锁定的硬约束）：对时效性主题先检索再讲，并标注信息时间与出处。
4. **词级时间轴 TTS**：换云端 TTS，用词级时间戳让板书要点踩着讲解节拍逐个亮起。
